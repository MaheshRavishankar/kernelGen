...
def find_bench_binary(build_dir: Path) -> Path:
    binary = build_dir / "gemm" / "kernel_providers" / "iree" / "iree_gemm_bench"
...
def run_one_test(
    ...
    # Generate MLIR and compile to .vmfb (cached in ~/.cache/kernelgen/).
    cache_dir = Path(
        os.environ.get("KERNELGEN_CACHE_DIR", Path.home() / ".cache" / "kernelgen")
    )
    vmfb_cache = cache_dir / "vmfb" / gpu_target
...
def main():
    parser = argparse.ArgumentParser(description="Run IREE GEMM benchmark")
    parser.add_argument(
    ...
    print(f"{'=' * 60}")
    print(f"IREE GEMM benchmark — {len(test_dirs)} test(s)")
    print(f"{'=' * 60}")
    print(
